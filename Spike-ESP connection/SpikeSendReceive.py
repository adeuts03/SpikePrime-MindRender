'''
Based on SpikeSendReceive.py, this code makes the SPIKE into a steering wheel with some force feedback.
...
Changelog
7/7/22
- Updated code, now it receives the speed from MR when the car hits something and then shakes
...
# # If dongle stuff starts giving you an error uncomment the following lines and see if 
# # the serial communication is working. If everything works properly then you should get
# # "Success" after dongle.setup() above and "Testing done." after the below block.
# file = '''
# print('testing')
# '''
# file = file.replace("\'",'"')
...
# The IP should be the laptop that is running MR. DO NOT change anything else - leave the quotes, 
...
while True:
    ...
    # Once we have data, we clean it up into something usable and turn the light on
    if not (ESPraw == ""): # Checks if MR sent data (which it always should)
        ...
        # MR is set to send 1s when there's no collision so we handle that here; if we 
        # have data, then we set that as the rumble factor and shake the wheel
        if not (MRdata == 1):
    ...
    # We send our motor position back to MR at all times (because we need to steer)
